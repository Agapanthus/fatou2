...
void main() {
    ...
    cloud =-
    // Richtige Wolken
    (voronoiAsy(st * vec2(4.0, 4.0) - seedX * 0.0, 0.005, asy) * 1.3 + 0.01 - 0.3 * cloudDensity);
    
    if (phase > phaseDistance * -6.0) {
        cloud =- (voronoiAsyFake(st * vec2(4.0, 4.0) - seedX * 0.0, 0.005, asy) * 1.3 + 0.01 - 0.3 * cloudDensity);
    }
    ...
    if (phase < phaseDistance * 2.0) {
        ...
        voronoiSmoothAsy(st * 9.0 - seedX * 0.2, 0.3, 1.0, 16.00) * 0.6;
        
        if (phase < phaseDistance * 4.0) {
            cloud -= voronoiAsy(st * 16.0 - seedX * 0.3 - seed * 1.0, - 0.1, 1.0) * 0.3;
