...
void main(){
    ...
    float cloud=-(
        ...
        (voronoiAsy(st*vec2(4.,4.)-seedX*0.4,.03,13.)*1.+.1)
        
        // Schäfchen, die sich langsam ineinander umformen
        +voronoi(st*9.-seed*0.3,.1)*.6
        +voronoi(st*16.-seedX*0.1,-.1)*.3
        
        // Weniger Schäfchen, mehr Chaos
        //   + 1. * (snoise(st * 8. - seed*2.) * .15
        //   + snoise(st * 16. - seed*2.) * .1)
        
        // Feine Strukturen
        //   + 1. * (snoise(st * 32. + seed*2.2)*0.03
        //    + voronoi(st * 32. + seed*2.2, .1) * 0.1
        //     + voronoi(st * 60. + seed*2.2, -.2) * -0.07)
    ...
    if(cloud>1.){
        ...
        gl_FragColor=vec4(mix(vec3(1.),color,0. + (-cloud+0.33)*9.),1.);//vec4(color*(1.-cloud)+vec3(cloud),1.);
    }else if(cloud>.15){
        gl_FragColor=vec4(mix(vec3(1.),color,max(.3,.3 + (-cloud+0.17)*9.)),1.);//vec4(color*(1.-cloud)+vec3(cloud),1.);
    }else if(cloud>0.){// 0.02
        gl_FragColor=vec4(mix(vec3(1.),color,max(.5,.5 + (-cloud+0.03)*9.)),1.);//vec4(color*(1.-cloud)+vec3(cloud),1.);
    }else if(cloud>0.){
        gl_FragColor=vec4(mix(vec3(0.,0.,0.),color,.5),1.);//vec4(color*(1.-cloud)+vec3(cloud),1.);
    }else{
