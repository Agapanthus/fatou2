...
float voronoiAsyLumo(vec2 st,float turb,float asy,float smoothness,vec2 lightDir){
    ...
    return m_lumo/m_dist*1.;// -(1./smoothness)*log(m_lumo);
    //return m_v * 200.0;
}
...
float fineStuff(vec2 st,vec2 seedX){
    return(
        ...
        +voronoiAsy(st*32.+seedX*.1,.1,1.)*.12
        +voronoiAsy(st*60.+seedX*.4,-.2,1.)*-.04
    );
}
...
void main(){
    ...
    float cloud=0.;
    float lightAngle= -10.*seed; // PI + atan(0.5-u_mouse.y/u_resolution.y,0.5-u_mouse.x/u_resolution.x); //-seed*9.;
    ...
    if(sqrt(pow2(gl_FragCoord.x/u_resolution.x-.05)+pow2(gl_FragCoord.y/u_resolution.y-.05))<.025){
        float lumo=max(dot(gl_FragCoord.xy/u_resolution.xy-vec2(.05),lightDir),0.);
        gl_FragColor=vec4(mix(vec3(.7),vec3(1.),(lumo*50.)),1.);
        ...
        cloud=-(
            ...
            +voronoiAsy(st*9.-seedX*.2,.4,1.)*.6
            +voronoiAsy(st*16.-seedX*.3-seed*1.,-.2,1.)*.3
            +details
        );
