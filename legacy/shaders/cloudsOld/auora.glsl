...
void main() {
    ...
    if (phase < phaseDistance * 0.0) {
        tlumi = shadowAt(st, seed, lightDir, 1.0, 1.0);
        tlumi *= shadowDark;
    } else if (phase < phaseDistance * -1.0) {
        tlumi = shadowAt(st, seed, lightDir, 1.0, 0.0);
        tlumi *= shadowDark;
    } else if (phase < phaseDistance * -2.0) {
        tlumi = shadowAt(st, seed, lightDir, 0.0, 00.0);
        tlumi *= shadowDark;
    }
    ...
    cloud =-
    // Richtige Wolken
    (voronoiAsy(st * vec2(4.0, 4.0) - seedX * 0.0, 0.0, asy) * 1.3 + 0.01 - 0.3 * cloudDensity);
    
    if (phase > phaseDistance * -6.0) {
        cloud =- (voronoiAsyFake(st * vec2(4.0, 4.0) - seedX * 0.0, 0.0, asy) * 1.3 + 0.01 - 0.3 * cloudDensity);
    }
    ...
    if (phase < phaseDistance * 2.0) {
        ...
        if (phase < phaseDistance * 4.0) {
            cloud -= voronoiAsy(st * 16.0 - seedX * 0.3 - seed * 1.0, - 0.2, 1.0) * 0.3;
    ...
    if (phase < phaseDistance * 6.0) {
        lightColor = mix(texColor, vec3(1.0), 0.0);
    }
    if (phase < phaseDistance * 8.0) {
        lightColor = whiteScale * Uncharted2Tonemap((log2(2.0 / pow(luminance / tlumi *1.9, 4.0))) * lightColor);
    ...
    // Add clouds
    retColor = mix(retColor, mix(lightColor, shadowColor, pow(tlumi*1.2, 1.3)), max(0.0, cloud));
