...
export class BakeableProgram extends Program {
    protected prog: SimpleProgram | null = null;
    protected cachedVertexShader: Shader | null = null;
    ...
    private resolveIncludes(s: string, context: { [name: string]: string }): string {
        s = this.preprocess(s, context);

        const visited: string[] = [];
        const getIncludesRe = (id: string, incs: string[]): string[][] => {
            let res: string[][] = [];
            visited.push(id);
            for (const inc of incs) {
                res.push([id, inc]);
                if (visited.includes(inc)) continue;
                const src = this.preprocess(
                    require("./shaders/preamble/" + inc + ".fs").default,
                    context
                );
        ...
        for (let inc of sortedIncs) {
            let src = s;
            if (inc !== null) {
                src = require("./shaders/preamble/" + inc + ".fs").default;
            } else inc = "main";
    ...
    public bake(useWebgl2: boolean, context: { [name: string]: string }) {
        ...
        if (useWebgl2) {
            ...
            fs = fs.replace(/^in\s+([^\n\;]+)\;\s*\n/gm, "varying $1;\n");
            fs = fs.replace(/^out\s+([^\n\;]+)\;\s*\n/gm, "\n");
            fs = fs.replace(/([^A-Za-z0-9\_]|^)fragColor([^A-Za-z0-9\_]|$)/gm, "$1gl_FragColor$2");
        }
