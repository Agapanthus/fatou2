...
string cef_version() {
    ...
    ver << "CEF: " << CEF_VERSION << " (Chromium: " << CHROME_VERSION_MAJOR
...
ProcessType GetProcessType(const CefRefPtr<CefCommandLine> &command_line) {
    ...
    const string &process_type = command_line->GetSwitchValue(kProcessType);
    if (process_type == kRendererProcess)
        return PROCESS_TYPE_RENDERER;

#if defined(OS_LINUX)
    // On Linux the zygote process is used to spawn other process types. Since
    // we don't know what type of process it will be we give it the renderer
    // app.
