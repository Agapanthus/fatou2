...
export abstract class Program {
    ...
    private uniformMap:
        {[name: string]: {loc: WebGLUniformLocation, val: any[]}} = {};
    ...
    protected buildUniformMap(uniforms: string[]) {
        ...
        for (const u of uniforms) {
            ...
            if (!this.uniformMap[u].loc)
                throw new Exception("Program.constructor uniform not found!",
                                    u);
        }
    }

    private find(name: string): WebGLUniformLocation {
        if (this.uniformMap[name])
            return this.uniformMap[name].loc;
    ...
    public set4f(name: string, value1: number, value2: number, value3: number,
                 value4: number): void {
    ...
    public set2d(name: string, value1: number, value2: number): void {
        this.gl.uniform4f(this.find(name), Math.fround(value1),
    ...
    public get(name: string) {
        if (this.uniformMap[name])
            return this.uniformMap[name].val;
        throw new Exception("Uniform not found", name);
    }
    ...
    protected buildRects(attributes: string[]) {
        ...
        for (const a of attributes) {
            ...
            if (!this.rects[a])
                throw new Exception("Program.constructor attribute not found!",
                                    a);
        }
    }
    ...
    public drawRect(name: string) { this.findRect(name).draw(); }
}