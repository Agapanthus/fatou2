...
class Optimizer {
    constructor() {
        this.sps = 10000; // 100000; // 30000; // 1000000
    ...
    public postRender(usedSamples: number, bufferCount: number, changes: number,
                      reset: boolean): void {
...
export class Progressive {
    constructor(gl: WebGLRenderingContext, tProg: Program, vProg: Program,
                target: Renderer, supersamples: number, win: Window) {
        this.gl = gl;
        this.tProg = tProg;
        this.vProg = vProg;
    ...
    public setProgram(renderToRect: (r: Rect) => void): void {
    ...
    public render(millis: number, continuos: boolean): boolean {
        ...
        if (!this.firstFrame) { // TODO: Frames nach Pausen müsssen
        ...
        if (this.currentLevel === this.upchain.length) {
            ...
            this.renderToRect(new Rect(0, 0, 1, 1));
        ...
        for (let dummy = 0; dummy < 20; dummy++) {
            ...
            if (bottom === this.currentProgress) { // You must do something!
                if (consumed === 0) {
            ...
            this.renderToRect(new Rect(0, this.currentProgress, 1, bottom));

            const consume =
                (bottom - this.currentProgress) * size.h * size.w / 2;
            left -= consume;
            consumed += consume;
        ...
        if (this.progress() === 0)
            this.win.setText("spp", this.spp() + " (max)", true, false);
        else if (this.liveChanges())
            this.win.setText("spp",
                             rationalize(this.spp()) + " -> " +
                                 rationalize((this.spp() * 2)) + " (live)",
                             true, false);
        else
            this.win.setText("spp",
                             rationalize(this.spp()) + " -> " +
                                 rationalize((this.spp() * 2)) + " (offscreen)",
                             true, false);
        ...
        this.optim.postRender(
            consumed, this.currentLevel, 0,
            false); // TODO: Die letzten beiden Parameter sind falsch...

        return changed;
    }

    // the overall progress - counting towards zero
    public progress() { return this.currentLevel - this.currentProgress }

    // current completed samples per pixels
    public spp() {
        return this.supersamples * this.supersamples /
               Math.pow(2, this.currentLevel)
    }
    ...
    private liveChanges(): boolean {
        ...
        // for medium spp
        return this.spp() > 1 / 8 && this.spp() < 4
    }
        ...
        if (this.currentLevel > 0) {
            ...
            if (this.liveChanges()) {
                ...
                if (this.currentLevel % 2 === 1)
                    dy = 0;
                this.tProg.setRect("aTexCoords", 0.0 + dx, 0.0 + dy, 1.0 + dx,
                                   1.0 + dy);
                this.tProg.drawRect("aPos");
    ...
    private setSize(size: Size): void {
        ...
        const tsize =
            new Size(size.w * this.supersamples, size.h * this.supersamples);
        this.down.setSize(tsize, size);
        let area: number = tsize.area();
        let vert: boolean = true;
        while (area > 2000) {
            if (vert) {
                this.upchain.push(new InterlacedProjection(tsize, this.gl, vert,
                                                           this.vProg, false));
    ...
    public setPush(pushFun: () => void): void { this.pushFunction = pushFun; }
    ...
    public getSupersamples() { return this.supersamples; }

    private currentProgress: number;
    private currentLevel:
        number; // the index of the largest finished buffer or upchain.length
    private upchain:
        InterlacedProjection[]; // the buffers from largest to smallest
    ...
    private renderToRect: (r: Rect) => void;
    private pushFunction: () => void = undefined;
