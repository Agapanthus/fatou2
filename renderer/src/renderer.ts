...
export class PlaceholderRenderer extends Renderer {
    constructor(placeholder: string) {
        ...
        this.gl =
            this.getCanvas().getContext("webgl2", {stencil : true}) as
            WebGLRenderingContext; // "experimental-webgl" // TODO: What to use
                                   // here?! Webgl2 is needed for bitwise logic,
                                   // but only supported by ca. 80% of devices!
        if (!this.gl.getContextAttributes().stencil)
            throw new f.Exception("No Stencilbuffer",
        ...
        this.refresh();
        this.getSize = () => {
            return new f.Size(this.gl.drawingBufferWidth,
...
export class Projection extends Renderer {
    constructor(size: f.Size, gl: WebGLRenderingContext,
                useRenderbuffer: boolean = false, linear: boolean = true) {
        super();
        this.gl = gl;
        ...
        if (typeof gl.checkFramebufferStatus ===
            "function") { // Not supported in IE and Edge
            if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !==
                gl.FRAMEBUFFER_COMPLETE)
                throw new f.Exception(
                    "Projection.constructor",
                    "Framebuffer: " +
                        gl.checkFramebufferStatus(gl.FRAMEBUFFER).toString());
        }
        ...
        this.getSize = () => { return this.size.clone(); };
        this.render = () => { this.writeTo(); };
    }
    ...
    public scale(size: f.Size): void { this.scaleI(size); }

    private scaleI(size: f.Size): void {
        this.size = size.clone();
        const gl = this.gl;
        gl.bindTexture(gl.TEXTURE_2D, this.texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, this.size.w, this.size.h, 0,
...
export class InterlacedProjection extends Projection {
    constructor(size: f.Size, gl: WebGLRenderingContext, vertical: boolean,
                prog: Program, linear: boolean) {
    ...
    public isVertical(): boolean { return this.vertical; }
    ...
    private createRenderBuffer() {
        ...
        if (this.vertical) {
            ...
            const s = 2.0 / w;
            for (let i = 0; i < w; i += 2) {
                this.prog.setRect("aPos", s * i - 1.0, -1.0, s * (i + 1) - 1.0,
                                  1.0);
            ...
            const s = 2.0 / h;
            for (let i = 0; i < h; i += 2) {
                this.prog.setRect("aPos", -1.0, s * i - 1.0, 1.0,
                                  s * (i + 1) - 1.0);
                this.prog.drawRect("aPos");
            }
        }
...
export class RenderLoop {
    constructor(renderF: ((delta: number, continuos: boolean) => boolean),
                drawF: () => void, onSleep: () => void) {
        this.renderF = renderF;
        this.drawF = drawF;
        this.onSleep = onSleep;

        requestAnimationFrame((millis: number) => { this.render(millis); });
    }
    ...
    public push(): void {
        if (this.onTerminate)
            console.error("Pushing a dead loop!");
        else
            this.active = true;
    }

    public fps(): number {
        if (this.floatingDelta < 0)
            return null;
    ...
    private render(millis: number) {
        ...
        if (this.onTerminate) {
            ...
            requestAnimationFrame((mi: number) => { this.render(mi); });
        }
        ...
        if (ac) {
            ...
            const ratio =
                Math.min(this.framesAwake, fpsSmoothing) / (fpsSmoothing + 1);
            this.floatingDelta =
                ratio * this.floatingDelta + (1 - ratio) * delta;
            this.framesAwake++;
        } else {
            this.framesAwake = 0;
    ...
    private floatingDelta: number =
        -1; // Smoothed medium frame-length in this awak-period.
    private framesAwake: number = 0; // number of frames since last sleep

    public terminate(): Promise<void> {
        this.active = false;
        return new Promise((res, rej) => {this.onTerminate = () => { res(); }})
    }
}