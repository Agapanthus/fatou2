...
import {finishLoading, loading, Rect} from "./global";
...
// @ts-ignore
import * as texture_shader from "./shaders/util/texture.fs"
// @ts-ignore
import * as white_shader from "./shaders/util/white.fs"
// @ts-ignore
import * as vertex_default from "./shaders/vertex/default.vs"
// @ts-ignore
import * as pos_shader from "./shaders/vertex/pos.vs"
...
export class FractalRunner {
    ...
    constructor() {
        ...
        this.textureProg =
            new SimpleProgram(vertex_default, texture_shader, gl,
                              [ "screenTexture" ], [ "aPos", "aTexCoords" ]);
        ...
        this.vProg = new SimpleProgram(pos_shader, white_shader, gl, [] as any,
                                       [ "aPos" ]);
        this.vProg.setRect("aPos", 0.0, 0.0, 1.0, 1.0);

        const addPresets =
            (name: string, fracClass: typeof Fractal, cons: () => Fractal) => {
        ...
        addPresets("Test", TestFractal, () => { return new TestFractal(); });
        ...
        this.fracClass = Antifractal;
        this.setFractal(new Antifractal())
    }
    ...
    private showFractal() {
        ...
        if (this.navi)
            this.navi.destroy();
        this.navi =
            new Navigator(this.renderer.getCanvas(), this.frac.defaults());

        // const p = new Projection(new Size(256, 256), gl);
        if (this.progressive)
            this.progressive.destroy();
        this.progressive = new Progressive(
            gl, this.textureProg, this.vProg, this.renderer,
            this.frac.defaults().superSamples || 4, this.dataWin);
        this.renderer.onResize = () => { this.progressive.refreshSize(); };
        this.progressive.setProgram((r: Rect) => {
            this.frac.use();
            this.frac.setRect(r);

            this.frac.setPos(this.navi.getPosX(), this.navi.getPosY());
            this.frac.setZoom(this.navi.getZoom(),
                              this.navi.getZoom() * this.renderer.getSize().h /
                                  this.renderer.getSize().w);

            this.dataWin.setText(
                "zoom", Math.log2(this.navi.getZoom()).toFixed(3) + " (log)");
        ...
        this.renderLoop = new RenderLoop(
            (delta: number, continuos: boolean) => {
                ...
                if (this.renderLoop.fps() === null)
                    ...
                    this.dataWin.setText(
                        "FPS", this.renderLoop.fps().toFixed(1), true, false);
        ...
        this.navi.setOnChangeFun(
            this.progressive.change.bind(this.progressive));
